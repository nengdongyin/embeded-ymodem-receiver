...
ymodem_receiver/
├── src/                                  # 核心协议库（跨平台、零依赖）
│   ├── ymodem_common.h  (142 行)         # 公共定义：帧结构宏、协议枚举、CRC API、系统时间接口
│   ├── ymodem_common.c  ( 71 行)         # CRC-16 CCITT 查表计算（256 项预计算表）
│   ├── ymodem_sender.h  (217 行)         # 发送端数据结构、事件枚举、API 声明
│   ├── ymodem_sender.c  (704 行)         # 发送端状态机 + 帧构建 + poll 超时
│   ├── ymodem_receiver.h(208 行)         # 接收端数据结构、事件枚举、API 声明
│   └── ymodem_receiver.c(804 行)         # 接收端逐字节解析器 + 延迟复位 + poll
│
├── test/
...
**核心库总代码量**：约 1946 行 C 代码，不依赖任何系统头文件（除 `<stdint.h>` 和 `<stdbool.h>`）。
...
```
IDLE
    │ ymodem_receiver_start() → 发送 'C'
    ▼
ESTABLISHING ── 等待 SOH 帧 (seq=0)
    │
...
CANCEL 路径（任意阶段）：
  CAN → WAIT_CAN_2 → CAN → ABORTED → IDLE
```
...
poll() 检查:
  if (now - last_time_ms >= YMODEM_TIMEOUT_MS):
    ...
    frame_stage_process() → frame_error_process()
      ...
      └── 超限    → 发送 CAN-CAN → ABORTED → 通知用户 ERROR
```
...
| 重传超限 | 发送 CAN-CAN，进入 ABORTED | 防止死锁 |
...
```
ymodem_sender_create()        ← 必须最先调用
ymodem_sender_set_event_callback()
ymodem_sender_set_send_packet_callback()
ymodem_sender_enable_1k()     ← 可选，启用 1K 模式
ymodem_sender_start()         ← 启动传输
    ┌─────────────────┐
    │ ymodem_sender_parse()  ← 循环调用，喂入接收端响应字节
    │ ymodem_sender_poll()   ← 循环调用，超时检测
    └─────────────────┘
```
...
```
ymodem_receiver_create()        ← 必须最先调用
ymodem_receiver_set_event_callback()
ymodem_receiver_set_send_response_callback()
ymodem_receiver_start()         ← 启动，自动发送初始 'C'
    ┌─────────────────┐
    │ ymodem_receiver_parse()  ← 循环调用，喂入接收字节
    │ ymodem_receiver_poll()   ← 循环调用，超时检测
    └─────────────────┘
```
...
| `test_sender.c` | 38 | 初始化/空指针防护/ESTABLISHING/ESTABLISHED/TRANSFERRING/FINISHING/FINISHED/CAN取消/超时重传/重传超限/1K模式/ABORTED重启/多文件/file_index递增验证 |
| `test_receiver.c` | 49 | 初始化/空指针防护/噪声忽略/帧头检测/SEQ验证/CRC错误/重传检测/重传超限/逐字节+分块解析/数据事件字段/最后一包截断/文件信息边界/EOT路径/poll超时/握手超限/多文件/会话结束/延迟复位验证/状态保留 |
| **总计** | **99** | |
